...
$named = $tabs;
$named[1]['label'] = 'Reading & Notes + Café';
set_theme_mod( 'koji_d3_home_tabs', $named );
$q = tabs_query( array(), 'Reading & Notes + Café' );
tabs_check( $q->get('category__in') === array($b), 'Actual name selects the tab with spaces, punctuation and Unicode' );
$url = get_pagenum_link(2, false);
parse_str( parse_url( $url, PHP_URL_QUERY ), $params );
tabs_check( $params['tab'] === $named[1]['label'], 'Pagination encodes the exact name once' );
ob_start(); koji_d3_render_home_tabs(); $nav = ob_get_clean();
tabs_check( false !== strpos($nav, 'tab=Reading%20%26%20Notes%20%2B%20Caf%C3%A9'), 'Navigation uses encoded real names' );
tabs_check( tabs_query(array(), 'links')->get('category__in') === array($b), 'Legacy ID bookmarks still select the tab' );
$duplicates = array( $tabs[0], array_merge($tabs[1], array('label'=>' writing ')) );
tabs_check( koji_d3_validate_home_tabs(new WP_Error(), $duplicates)->has_errors(), 'Duplicate names are rejected before saving' );
tabs_check( count(koji_d3_sanitize_home_tabs($duplicates)) === 1, 'Sanitization enforces name uniqueness' );
tabs_check( ! koji_d3_validate_home_tabs(new WP_Error(), $named)->has_errors(), 'Distinct names pass validation' );
$named[1]['label'] = '   ';
tabs_check( koji_d3_validate_home_tabs(new WP_Error(), $named)->has_errors(), 'Blank names are rejected' );
set_theme_mod( 'koji_d3_home_tabs', $tabs );
$chosen = $tabs;
