...
function koji_d3_register_home_tabs_customizer( $wp_customize ) {
	class Koji_D3_Home_Tabs_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<p><?php esc_html_e( 'Check Default tab on the tab to show first when visiting the homepage. Only one tab can be the default. Add up to 20 tabs, then use Move up or Move down to set their order. Names must be unique (ignoring letter case). Names appear in URLs, so renaming a tab changes its URL. Publish to save.', 'koji-d3' ); ?></p>
	...
	$wp_customize->add_setting( 'koji_d3_home_tabs', array( 'default' => array(), 'sanitize_callback' => 'koji_d3_sanitize_home_tabs', 'validate_callback' => 'koji_d3_validate_home_tabs' ) );
